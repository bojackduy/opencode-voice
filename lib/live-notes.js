...
import { createWhisperServerClient } from "./whisper-server.js";
...
export function registerLiveNotes(api, kv, complete, opts, logger, deps = {}) {
  ...
  async function startNotes() {
    ...
    if (!useApiMode && useWhisperServer) {
      whisperServerClient = createWhisperServerClient({ modelPath, language, logger });
