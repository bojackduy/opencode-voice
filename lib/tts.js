...
export function registerTTS(api, kv, complete, prompts, opts, logger, deps = {}) {
  ...
  let wasBusy = false;
  // Set by the voice-conversation loop while it owns speaking. Auto TTS stays
  // out of the way (and consumes the busy flag) so replies are not spoken twice.
  ...
  async function speakAssistantTurn() {
    ...
    const llmResult = await normalizeForSpeech(result.text, systemAuto, 2048);
