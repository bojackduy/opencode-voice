...
export function registerTTS(api, kv, complete, prompts, opts, logger) {
  ...
  function segmentByLang(text) {
    ...
    let curLang = null;
    let curText = "";
    // split into tokens: keep words, punctuation, code, spaces
    // heuristic: each token classified, consecutive same lang merged
    const tokens = text.split(
      new RegExp(
        "(\\s+|[a-zA-Z_][a-zA-Z0-9_./]*|[àáảãạăắằẳẵặâấầẩẫậèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵđĐ]+)",
        "g",
      ),
    );
    for (const tok of tokens) {
      if (!tok) continue;
      const lang = VI_REGEX.test(tok) ? "vi" : /^[a-zA-Z]/.test(tok) ? "en" : curLang || "en";
      // for punctuation/spaces, keep with previous lang
      const tokLang = tok.trim() === "" || /^[\s.,!?;:()"'`]+$/.test(tok) ? curLang || lang : lang;
      if (tokLang !== curLang && curText) {
        segments.push({ text: curText, lang: curLang });
        curText = tok;
        curLang = tokLang;
      } else {
        if (!curLang) curLang = tokLang;
        curText += tok;
      }
    }
    if (curText) segments.push({ text: curText, lang: curLang || "en" });
    // merge single-char segments to avoid choppy synthesis
    const merged = [];
    for (const s of segments) {
      if (merged.length && s.text.length <= 2 && s.lang === merged[merged.length - 1].lang) {
        merged[merged.length - 1].text += s.text;
      } else if (merged.length && s.text.trim().length <= 1) {
        merged[merged.length - 1].text += s.text;
      } else {
        merged.push(s);
      }
    }
    // fallback: if only one segment, keep as is
    if (merged.length === 0) return [{ text, lang: detectLang(text) }];
    // if mixed but all en or all vi, keep single
    if (merged.length > 1 && merged.every((s) => s.lang === merged[0].lang)) {
      return [{ text, lang: merged[0].lang }];
    }
    return merged;
  }
  ...
  async function speakOne(line, voiceModel) {
  ...
  async function speak(text) {
    ...
    // mixed vi+en: split and speak sequentially with correct voices
    const segments = segmentByLang(line);
    const isMixed =
      segments.length > 1 &&
      segments.some((s) => s.lang === "vi") &&
      segments.some((s) => s.lang === "en");
    if (isMixed) {
      logger?.log?.(
        "TTS",
        `Mixed vi/en detected segments=${segments.map((s) => `${s.lang}:${s.text.slice(0, 20)}`).join(" | ")}`,
        "debug",
      );
      for (const seg of segments) {
        const voiceModel = getVoiceModel(seg.text);
        logger?.log?.(
          "TTS",
          `Speak segment lang=${seg.lang} chars=${seg.text.length} voice=${voiceModel}`,
          "debug",
        );
        if (!piperOnPath()) {
        ...
        await speakOne(seg.text, voiceModel);
        // check if stop was requested mid-sequence
        if (!piperProc && !playProc && segments.indexOf(seg) !== segments.length - 1) {
          // continue to next
        }
      }
      return;
    ...
    return speakOne(line, voiceModel);
  }
