...
export function buildRecordingMessage() {
  if (!useWave || recordingStartMs === null)
    return `${RECORDING_TOAST_BASE} · ${currentRecordKeybind} to stop`;
  const elapsed = formatElapsed(Date.now() - recordingStartMs);
  const wave = getWaveString();
  return `${RECORDING_TOAST_BASE}  ${wave}  ${elapsed} · ${currentRecordKeybind} to stop`;
}
...
function startRecording(kv, backend, toast, logger) {
  ...
  soxProc = spawn(
    "sox",
    [...inputArgs, "-r", "16000", "-c", "1", "-b", "16", wavFile, "silence", "1", "0.05", "1%"],
    {
...
function transcribe(kv, logger) {
  ...
  try {
    ...
    logger?.log(
      "STT",
      `Local transcription model=${mp} lang=${lang} mem free=${freeGB}GB/${totalGB}GB`,
      "debug",
    );
    if (parseFloat(freeGB) < 4)
      logger?.log(
        "STT",
        `Low free RAM ${freeGB}GB — large models may swap, consider sttModel medium or sttEndpoint API`,
        "warn",
      );
  } catch {}
...
For Vietnamese (vi): keep diacritics exact (e.g., "tim" vs "tìm", "doc" vs "đọc", "sua" vs "sửa") and do not strip tones.

Rely heavily on context to fix words that sound similar to programming terminology.

Model/RAM note: on 36GB Mac with ~30GB used, ~6GB free fits large-v3-turbo-q5_0 (~0.8GB) but q8_0/full (~1.1-3GB) may swap. If whisper encode >2s, consider tui.json "sttEndpoint" API offload to save RAM.`;
