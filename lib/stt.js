...
export function streamModelKey(modelPath, language) {
  return `${modelPath || ""}::${language || "auto"}`;
}
...
export function registerSTT(api, kv, complete, prompts, opts, logger, deps = {}) {
  ...
  function currentStreamModelKey() {
    return streamModelKey(getModelPath(kv), getLanguage(kv));
  }
  ...
  function retainWarmStreamController(controller) {
    ...
    warmStreamModelKey = currentStreamModelKey();
  }
  ...
  async function startStreamingDictation() {
    ...
    // start loads the new model instead of reusing the stale server.
    if (warmStreamController && warmStreamModelKey !== currentStreamModelKey()) {
    ...
    try {
      ...
      toast("● Streaming dictation - /stt-record to finalize, /stt-stop to cancel");
