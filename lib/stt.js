...
export function rmsToChar(rms) {
  // more sensitive: quiet speech ~0.015 now hits mid bars
  if (rms < 0.005) return WAVE_CHARS[0];
  if (rms < 0.012) return WAVE_CHARS[1];
  if (rms < 0.025) return WAVE_CHARS[2];
  if (rms < 0.05) return WAVE_CHARS[3];
  if (rms < 0.085) return WAVE_CHARS[4];
  if (rms < 0.13) return WAVE_CHARS[5];
  if (rms < 0.2) return WAVE_CHARS[6];
  return WAVE_CHARS[7];
}

function fakePulseChar() {
  // faster, multi-frequency pulse so still looks alive when mic silent
  const t = Date.now() / 120;
  const v = (Math.sin(t) + 0.5 * Math.sin(t * 2.3) + 1.5) / 3;
  const idx = Math.floor(v * (WAVE_CHARS.length - 1));
...
export function computeWaveChar() {
  ...
  try {
    ...
    if (data.length <= 44) return fakePulseChar();
    // shorter window ~0.08s = 2560 bytes → much more instant response
    ...
    if (samples === 0) return fakePulseChar();
    let sum = 0;
    ...
    // very quiet (<0.003) still pulse; 0.003-0.005 already maps to ▁/▂ so keep real
    if (rms < 0.003) return fakePulseChar();
    return rmsToChar(rms);
  } catch {
    return fakePulseChar();
  }
}

export function getWaveString() {
  const ch = computeWaveChar();
  waveHistory.push(ch);
  if (waveHistory.length > WAVE_LEN) waveHistory.shift();
  // pad with low blocks at start
  while (waveHistory.length < WAVE_LEN) waveHistory.unshift(WAVE_CHARS[0]);
  return waveHistory.join("");
}
...
export function showRecordingToast() {
  ...
  waveHistory = Array(WAVE_LEN).fill(WAVE_CHARS[0]);
...
function fallbackForRaw(raw) {
  const t = raw.trim();
  if (!t) return t;
  // minimal cleanup: capitalize first letter, ensure trailing period for greetings
