...
async function normalizeTranscription(complete, rawText, contextBlock, systemPrompt, logger) {
  logger?.log(
    "STT",
...
async function transcribeTurn(kv, complete, client, api, toast, systemPrompt, logger) {
  ...
  try {
    ...
    const contextBlock = [sessionTitle ? `[session: "${sessionTitle}"]` : null, recent || null]
      .filter(Boolean)
      .join("\n");
    ...
    logger?.log("STT", `Turn transcription ready chars=${llmResult.text.length}`, "debug");
