...
async function doTranscribePipeline(
  ...
  try {
    ...
    if (!llmResult.text) {
      ...
      await appendTranscription(client, renderer, result.text, submit);
      return;
    }

    await appendTranscription(client, renderer, llmResult.text, submit);
  ...
  return [
    {
      ...
      onSelect() {
        ...
        if (recording) {
          ...
          doTranscribePipeline(kv, complete, client, toast, systemPrompt, false, logger, renderer);
        } else {
    ...
    },
    {
      ...
      onSelect() {
        ...
        clearRecordingToast();
        toast("Stopping, transcribing...");
        doTranscribePipeline(kv, complete, client, toast, systemPrompt, true, logger, renderer);
      },
    },
    {
