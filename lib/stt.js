...
export function rmsToChar(rms) {
  // tuned for 16kHz mono: quiet 0.008→▂, normal 0.03→▅, loud 0.15→█
  if (rms < 0.004) return WAVE_CHARS[0];
  if (rms < 0.01) return WAVE_CHARS[1];
  if (rms < 0.022) return WAVE_CHARS[2];
  if (rms < 0.045) return WAVE_CHARS[3];
  if (rms < 0.08) return WAVE_CHARS[4];
...
export function rmsToLevel(rms) {
  ...
  if (rms < 0.022) return 2;
  if (rms < 0.045) return 3;
  if (rms < 0.08) return 4;
  if (rms < 0.13) return 5;
  if (rms < 0.2) return 6;
  return 7;
}
...
export function getWaveString() {
  // Fixed-column snapshot: each column = RMS of sub-slice of last ~0.16s window
  const wavFile = path.join(tmpDir, WAV_FILENAME);
  try {
    const data = fs.readFileSync(wavFile);
    if (data.length <= 44) {
    ...
    const windowSize = 5120; // ~0.16s at 16kHz mono 16-bit
    const start = Math.max(44, data.length - windowSize);
    const slice = data.subarray(start);
    ...
    for (let c = 0; c < WAVE_LEN; c++) {
      ...
      const rms = Math.sqrt(sum / samples);
      levels.push(rmsToLevel(rms));
    }
    // decay smoothing: fast attack, slow decay for punchy feel
    ...
    const smoothed = levels.map((lvl, i) => {
      ...
      const decayed = Math.floor(prev * 0.55);
      return lvl > prev ? lvl : Math.max(lvl, decayed);
    });
